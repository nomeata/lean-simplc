...
simp_lc ignore eq_self
simp_lc ignore sizeOf_default

-- (i : Nonempty b) ⊢ b
simp_lc whitelist imp_self forall_const

-- Missing List.join_append
-- simp_lc inspect List.append_bind List.bind_id

@[simp] theorem Char.toNat_mk : Char.toNat ⟨v,n⟩ = v.toNat := rfl
-- needed for (without ac_rfl)
-- simp_lc inspect Char.mk.sizeOf_spec Char.sizeOf by simp_all; ac_rfl
...
simp_lc ignore List.mem_of_find?_eq_some

-- variant of add_one_le_iff
@[simp] theorem add_one_eq_iff {n : Nat} : ∀ {k : Fin (n + 1)}, k + 1 = k ↔ k = Fin.last _ := by
  sorry
-- for
-- simp_lc inspect Fin.le_zero_iff Fin.add_one_le_iff
...
-- @[simp] theorem bne_not_right (x y : Bool) : (x != (!y)) = ((!x) != y) := sorry
-- for
-- simp_lc inspect Bool.bne_assoc Bool.bne_true

simp_lc whitelist Bool.bne_assoc bne_self_eq_false'
simp_lc whitelist Bool.bne_assoc Bool.bne_self_left
simp_lc whitelist Bool.bne_assoc Bool.bne_not_self
...
theorem Function.comp.assoc (f : φ → δ) (g : β → φ) (h : α → β) : (f ∘ g) ∘ h = f ∘ g ∘ h :=
  rfl
-- fixes
-- simp_lc check List.map_map List.map_map

@[simp] theorem takeWhile_nil : List.takeWhile p [] = [] := rfl
-- fixes
-- simp_lc check List.takeWhile_append_dropWhile List.dropWhile_nil

attribute [simp] or_assoc
attribute [simp] and_assoc
-- fixes
-- simp_lc check List.mem_append List.cons_append

-- Bug: tactic 'rfl' failed, equality expected
-- simp_lc check List.getElem?_eq_get? List.getElem?_eq_get?

-- Not a good lemma: Non-linear, and goes against the reduction direction
attribute [-simp] List.get_cons_drop
-- simp_lc check List.any_cons List.get_cons_drop

-- Unclear which one is at fault here
simp_lc whitelist List.head?_reverse List.reverse_cons
simp_lc whitelist List.head?_reverse List.reverse_append

-- This should be resolvable using List.mem_cons, but
-- because of the dependency in `decide (x ∈ a :: as)`
simp_lc whitelist List.elem_eq_mem List.contains_cons


-- We need the non-linear version of List.drop_left
attribute [-simp] List.drop_left
attribute [simp] List.drop_left'
-- simp_lc check List.drop_left List.length_replicate
...
attribute [simp] Nat.add_assoc'
-- simp_lc check List.drop_drop' List.drop_drop'
...
-- simp_lc check List.drop_drop' List.drop_left'
...
-- simp_lc check List.get?_drop' List.drop_succ_cons

-- Why does this not work:
-- attribute [simp] List.append_ne_nil_of_ne_nil_left
-- attribute [simp] List.append_ne_nil_of_ne_nil_right
-- -- set_option trace.Meta.Tactic.simp true in
-- simp_lc check List.dropLast_append_of_ne_nil List.cons_append
